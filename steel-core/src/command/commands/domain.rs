...
use steel_registry::data_components::vanilla_components::{
    CUSTOM_NAME, ENCHANTMENT_GLINT_OVERRIDE,
};
...
pub fn command_handler() -> impl CommandHandlerDyn {
    ...
    .executes(|(), ctx: &mut CommandContext| {
        ...
        player.open_menu("Domains", |id, current_world| {
            ...
            for (i, name) in domains.into_iter().enumerate() {
                ...
                for (j, world) in worlds.iter().enumerate() {
                    ...
                    icon.set(CUSTOM_NAME, world.key.to_string().into());
...
impl MenuKind for DomainMenuKind {
    fn on_slot_clicked(
        ...
        let Some((_ident, world)) = self.map.get_key_value(&index) else {
